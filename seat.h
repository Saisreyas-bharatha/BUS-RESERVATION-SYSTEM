...
void nodecreation(int i){
	...
	for(j=0;j<BUS_NUM;j++){
		if(j==i){
			for(k=0;k<TOTAL_SEATS;k++){   //in this 32 empty nodes(seats) get created
				...
				cur->id=NULL;
				if(first==NULL){
					first=cur;  //if no node is present then isolated node cur gets assgined to first -> first node is created 
					cur->link=NULL; 
					last=cur;
					struct Bus *bus = (struct Bus *)malloc(sizeof(struct Bus));   //as malloc returns void pointer it gets typecasted explicitly to struct Bus data type
                    bus->unqID = i;
...
void seat(char from[100],char to[100],char day[100]){
	...
	while(1){
		...
		switch(ch){
			case 1:
				k=0;   //no of times k gets incremented while loop runs
                if (arr[i] == NULL) {
                    nodecreation(i);
                }
...
void create(int c,char from[100],char to[100],char day1[100]){
	...
	char re[100],a2[10];
	struct Passenger *t=arr[c]->firstPassenger;  //first node of arr[c](bus no) gets assgined to t
        ...
        if(j1==c){
	        for(seatNo=1;seatNo<=TOTAL_SEATS;seatNo++){
	        	if(strcmp(t2->name," ")==0 && strcmp(t2->phoneno," ")==0 && t2->tid==NULL){
					printf("[%02d]",seatNo);
					c1++;  //display of seats in the required manner
					c2++;
				}
				else{
					printf("[XX]");
